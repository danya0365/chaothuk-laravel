                ...
                <div class="flex flex-col">
                    <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
                        ...
                        <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
                            ...
                            <form action="{{ route('supervisor.work-types.update', $workType->id) }}" method="POST">
                                {{ method_field('PATCH') }}
                                @csrf
                                <div class="flex flex-col space-y-4">
                                    <div>
                                        <label class="block text-sm font-bold text-gray-700" for="title">Title</label>
                                        <input value="{{ $workType->title }}" type="text" name="title" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Title">
                                    </div>
