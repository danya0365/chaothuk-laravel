<div class="p-6 sm:p-10 max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">จัดการประกาศจ้างงาน (Recruits)</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">ดูแลรายการประกาศตามหาคนทำงานหรือรับสมัครงานที่ผู้ใช้โพสต์ลงในระบบ</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div class="w-full md:w-1/3 relative">
            ...
            <input wire:model.live.debounce.500ms="search" type="text" placeholder="ค้นหาชื่อประกาศ..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition">
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">ชื่อประกาศ</th>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">งบประมาณ</th>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0">
                                        @if($recruit->primary_image)
                                            <img class="h-10 w-10 rounded-md object-cover" src="{{ str_contains($recruit->primary_image, 'http') ? $recruit->primary_image : Storage::url($recruit->primary_image) }}" alt="">
                                        @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-300">
                                    {{ $recruit->categories->pluck('name')->join(', ') ?: '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">
                                ฿{{ number_format($recruit->budget, 2) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="text-sm text-gray-900 dark:text-white">
                                        <a href="{{ route('backend.users.show', $recruit->author_id) }}" class="hover:underline text-indigo-600 dark:text-indigo-400" target="_blank">{{ $recruit->author->name }}</a>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <!-- Feature Button -->
                                    <button wire:click="toggleFeature({{ $recruit->id }})" class="{{ $recruit->display_priority > 0 ? 'text-amber-600 hover:text-amber-900 dark:text-amber-500 dark:hover:text-amber-400' : 'text-gray-400 hover:text-amber-600 dark:hover:text-amber-400' }}" title="{{ $recruit->display_priority > 0 ? 'เลิกแนtนำ' : 'ตั้งเป็นรายการแนะนำ' }}">
                                        <svg class="h-5 w-5" fill="{{ $recruit->display_priority > 0 ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                                    </button>

                                    <!-- Suspend Button -->
                                    <button wire:click="toggleSuspend({{ $recruit->id }})" class="{{ $recruit->is_suspended ? 'text-green-600 hover:text-green-900 dark:text-green-500 dark:hover:text-green-400' : 'text-red-500 hover:text-red-700 dark:hover:text-red-400' }}" title="{{ $recruit->is_suspended ? 'ยกเลิกการระงับ' : 'ระงับการแสดงผล' }}">
                                        @if($recruit->is_suspended)
                                    ...
                                    <a href="{{ route('backend.recruits.show', $recruit->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    </a>

                                    <!-- Delete -->
                                    <button wire:click="confirmDelete({{ $recruit->id }})" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full p-6">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">ลบรายการประกาศ</h3>
                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                คุณแน่ใจหรือไม่ที่จะลบรายการประกาศจ้างงานนี้? การกระทำนี้ไม่สามารถกู้คืนได้ (เนื้อหาจะถูกเก็บในถังขยะ: Soft Delete)
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <button wire:click="deleteRecruit" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">ยืนยันการลบ</button>
