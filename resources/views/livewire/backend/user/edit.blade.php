<div class="p-6 sm:p-10 max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">แก้ไขข้อมูลสมาชิก</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">อัปเดตข้อมูลของบัญชี: {{ $user->name }}</p>
        </div>
    </div>
        ...
        <form wire:submit="save" class="p-6 sm:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="mt-2 flex items-center space-x-4">
                        <div class="h-16 w-16 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex-shrink-0">
                            ...
                            @elseif ($user->profile_image)
                                <img src="{{ image_url($user->profile_image) }}" class="h-full w-full object-cover">
                            @else
                    ...
                    <div class="mt-2 flex items-center space-x-4">
                        <div class="h-16 w-32 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex-shrink-0">
                            ...
                            @elseif ($user->cover_image)
                                <img src="{{ image_url($user->cover_image) }}" class="h-full w-full object-cover">
                            @else
                ...
                <div>
                    ...
                    <input wire:model="email" type="email" id="email" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div>
                    ...
                    <input wire:model="mobile_phone" type="text" id="mobile_phone" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    @error('mobile_phone') <span class="text-sm text-red-600 dark:text-red-400 mt-1 block">{{ $message }}</span> @enderror
                </div>

                <!-- Password -->
                <div>
                    <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-4">รีเซ็ตรหัสผ่าน</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">หากไม่ต้องการเปลี่ยนรหัสผ่าน ให้เว้นช่องนี้ว่างไว้</p>
                    
                    <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">รหัสผ่านใหม่</label>
                    <input wire:model="password" type="password" id="password" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="เว้นว่างไว้หากไม่เปลี่ยน">
                    @error('password') <span class="text-sm text-red-600 dark:text-red-400 mt-1 block">{{ $message }}</span> @enderror
                </div>

                <!-- Confirm Password -->
                <div>
                    <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-4 invisible">รีเซ็ตรหัสผ่าน</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4 invisible">หากไม่ต้องการเปลี่ยนรหัสผ่าน ให้เว้นช่องนี้ว่างไว้</p>

                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700 dark:text-gray-300">ยืนยันรหัสผ่านใหม่</label>
                    <input wire:model="password_confirmation" type="password" id="password_confirmation" class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="เว้นว่างไว้หากไม่เปลี่ยน">
                </div>
            </div>
            ...
            <div class="pt-6 mt-6 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="inline-flex justify-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-900 transition">
                    ...
                    <span wire:loading.remove wire:target="save">อัปเดตข้อมูล</span>
