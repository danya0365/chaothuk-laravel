                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($badges as $badge)
                        <div class="relative bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between group">
                            ...
                            <button wire:click="removeBadge({{ $badge->id }})" wire:confirm="คุณแน่ใจหรือไม่ที่จะเพิกถอนตราสัญลักษณ์นี้ออกจากผู้ใช้?" class="absolute top-4 right-4 text-gray-400 hover:text-red-600 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity focus:outline-none">
