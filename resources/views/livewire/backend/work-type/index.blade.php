<div x-data="{ itemToDelete: null, isDeleteModalOpen: false }">
    <x-slot name="title">ประเภทงาน (Work Types)</x-slot>

    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">ประเภทงาน (Work Types)</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">จัดการหมวดหมู่หรือประเภทของงานในระบบ</p>
        </div>
        <div>
            <a href="{{ route('backend.work-types.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring focus:ring-indigo-300 disabled:opacity-25 transition">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">ภาพ/ไอคอน</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">ชื่อประเภทงาน</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($workTypes as $type)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($type->image)
                                    <div class="h-10 w-10 flex-shrink-0">
                                        <img class="h-10 w-10 rounded-md object-cover" src="{{ image_url($type->image) }}" alt="">
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $type->title }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    <a href="{{ route('backend.work-types.edit', $type->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">
                                    ...
                                    <button 
                                        type="button"
                                        @click="itemToDelete = {{ $type->id }}; isDeleteModalOpen = true"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <span>ไม่พบข้อมูลประเภทงาน</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white" id="modal-title">ยืนยันการลบประเภทงาน</h3>
                            <div class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                <p>คุณแน่ใจหรือไม่ว่าต้องการลบประเภทงานนี้? การกระทำนี้ไม่สามารถย้อนกลับได้</p>
                            </div>
                        </div>
                    </div>
                </div>
