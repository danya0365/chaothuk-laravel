                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                ...
                                <div class="flex items-center">
                                    <div class="h-10 w-10 flex-shrink-0">
                                        @if($work->primary_image)
                                            <img class="h-10 w-10 rounded-md object-cover" src="{{ str_contains($work->primary_image, 'http') ? $work->primary_image : Storage::url($work->primary_image) }}" alt="">
                                        @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <!-- Feature Button -->
                                    <button wire:click="toggleFeature({{ $work->id }})" class="{{ $work->display_priority > 0 ? 'text-amber-600 hover:text-amber-900 dark:text-amber-500 dark:hover:text-amber-400' : 'text-gray-400 hover:text-amber-600 dark:hover:text-amber-400' }}" title="{{ $work->display_priority > 0 ? 'เลิกแนtนำ' : 'ตั้งเป็นรายการแนะนำ' }}">
                                        <svg class="h-5 w-5" fill="{{ $work->display_priority > 0 ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                                    </button>
