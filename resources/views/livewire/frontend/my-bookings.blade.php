<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <h1 class="text-2xl font-bold text-white mb-6">📋 การจองของฉัน</h1>
    ...
    @if(count($bookings) === 0)
        <div class="text-center py-16">
            ...
            <p class="text-gray-400 font-medium">ยังไม่มีการจอง{{ $statusFilter !== 'all' ? ' ในสถานะนี้' : '' }}</p>
        </div>
    @else
        <div class="space-y-3">
            @foreach($bookings as $booking)
                ...
                @endphp
                <div class="bg-gray-900 rounded-xl p-4 border-l-4 {{ $statusColor }}">
                    <div class="flex items-start gap-4">
                        ...
                        @if(($booking['booking_status'] ?? '') === 'waiting-to-confirm')
                            <button wire:click="cancelBooking('{{ $tab }}', {{ $booking['id'] }})"
                                    ...
                                    class="px-3 py-1.5 text-xs bg-red-500/10 text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/20 transition flex-shrink-0">
                                ยกเลิก
                            </button>
                        @endif
                    </div>
                    @if($booking['customer_message'] ?? null)
                        <p class="text-gray-500 text-xs mt-2 italic pl-20">"{{ $booking['customer_message'] }}"</p>
                    @endif
                </div>
            @endforeach
        </div>
