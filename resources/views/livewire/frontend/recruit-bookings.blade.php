<div class="max-w-4xl mx-auto px-3 sm:px-4 py-4 md:py-6">
    ...
    {{-- Header --}}
    <div class="flex items-center justify-between gap-3 md:gap-4 mb-4 md:mb-6">
        <div class="flex items-center gap-2 md:gap-3">
            <a href="{{ route('frontend.recruits.show', $id) }}"
               class="text-xs md:text-sm text-gray-400 hover:text-white transition">← กลับ</a>
            <div>
                <h1 class="text-lg md:text-xl font-bold text-white">📋 รายการสมัครงาน</h1>
                <p class="text-gray-500 text-xs md:text-sm">{{ $recruit->title }}</p>
            </div>
        </div>
    </div>

    {{-- Stats Cards --}}
    <div class="grid grid-cols-3 md:grid-cols-5 gap-1.5 md:gap-2 mb-4 md:mb-5">
        @php
            $statItems = [
        ...
        @foreach($statItems as $s)
            <button wire:click="$set('filterStatus', '{{ $s['filter'] }}')"
                    class="rounded-xl p-2 md:p-3 text-center transition {{ $filterStatus === $s['filter'] ? 'ring-2 ring-green-500' : '' }} {{ $s['bg'] }}">
                <div class="text-base md:text-xl font-black {{ $s['color'] }}">{{ $s['value'] }}</div>
    ...
    <div class="space-y-3">
        @forelse($bookings as $booking)
            <div class="bg-gray-900 rounded-xl p-3 md:p-4 {{ $booking->booking_status === 'waiting-to-confirm' ? 'ring-1 ring-green-500/30' : '' }}">
                <div class="flex items-start gap-2.5 md:gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between mb-1.5 flex-wrap gap-1">
                            <span class="text-sm md:text-base text-white font-semibold">{{ $booking->author?->name ?? 'ผู้สมัคร' }}</span>
                            @php
                            ...
                            @endphp
                            <span class="text-[10px] md:text-xs font-semibold px-2 md:px-2.5 py-0.5 rounded-full ring-1 {{ $sc[0] }}">
                                {{ $sc[1] }}
                            </span>
                        </div>

                        <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs md:text-sm text-gray-400">
                            <span>📅 {{ $booking->booking_date ? \Carbon\Carbon::parse($booking->booking_date)->format('d/m/Y') : '-' }}</span>
                            <span>📞 {{ $booking->mobile_phone ?? '-' }}</span>
                            <span class="col-span-2">🕐 {{ $booking->created_at?->diffForHumans() }}</span>
                        </div>

                        @if($booking->customer_message)
                            <div class="mt-1.5 md:mt-2 bg-gray-800 rounded-lg px-2.5 py-1.5 md:px-3 md:py-2">
                        ...
                        @if($booking->booking_status === 'waiting-to-confirm')
                            <div class="flex gap-2 mt-2 md:mt-3">
                                <button wire:click="confirmBooking({{ $booking->id }})"
                                        wire:confirm="ยืนยันการสมัครนี้?"
                                        class="flex-1 sm:flex-none px-3 py-1.5 md:px-5 md:py-2 bg-green-600 hover:bg-green-500 text-white text-[10px] md:text-xs font-bold rounded-lg transition">
                                    ✅ ยืนยัน
                                </button>
                                <button wire:click="cancelBooking({{ $booking->id }})"
                                        wire:confirm="ต้องการปฏิเสธการสมัครนี้?"
                                        class="flex-1 sm:flex-none px-3 py-1.5 md:px-5 md:py-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 text-[10px] md:text-xs font-bold rounded-lg transition ring-1 ring-red-500/30">
                                    ❌ ปฏิเสธ
                                </button>
                            </div>
                        @elseif($booking->booking_status === 'confirm')
                            <div class="flex gap-2 mt-2 md:mt-3">
                                <button wire:click="closeBooking({{ $booking->id }})"
                                        wire:confirm="ปิดงานนี้? (เสร็จสิ้น)"
                                        class="flex-1 sm:flex-none px-3 py-1.5 md:px-5 md:py-2 bg-blue-600 hover:bg-blue-500 text-white text-[10px] md:text-xs font-bold rounded-lg transition">
                                    🔒 ปิดงาน (เสร็จสิ้น)
                                </button>
                                <button wire:click="cancelBooking({{ $booking->id }})"
                                        wire:confirm="ต้องการยกเลิก?"
                                        class="flex-1 sm:flex-none px-3 py-1.5 md:px-5 md:py-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 text-[10px] md:text-xs font-bold rounded-lg transition ring-1 ring-red-500/30">
                                    ❌ ยกเลิก
                                </button>
                            </div>
        ...
        @empty
            <div class="text-center py-10 md:py-16">
                ...
                <p class="text-gray-500 text-sm md:text-base">{{ $filterStatus ? 'ไม่พบรายการในสถานะนี้' : 'ยังไม่มีการสมัคร' }}</p>
            </div>
        @endforelse
    </div>
