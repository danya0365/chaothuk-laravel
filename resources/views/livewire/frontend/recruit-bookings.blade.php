<div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-white">📋 รายการสมัครงาน</h1>
                <p class="text-gray-500 text-sm">{{ $recruit->title }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-5 gap-2 mb-5">
        ...
        @foreach($statItems as $s)
            <button wire:click="$set('filterStatus', '{{ $s['filter'] }}')"
                    class="rounded-xl p-3 text-center transition {{ $filterStatus === $s['filter'] ? 'ring-2 ring-green-500' : '' }} {{ $s['bg'] }}">
    ...
    <div class="space-y-3">
        @forelse($bookings as $booking)
            <div class="bg-gray-900 rounded-xl p-4 {{ $booking->booking_status === 'waiting-to-confirm' ? 'ring-1 ring-green-500/30' : '' }}">
                <div class="flex items-start gap-3">
                    <img src="{{ $booking->author?->getAvatar(48) ?? '' }}"
                         class="w-11 h-11 rounded-full object-cover flex-shrink-0" alt="">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-white font-semibold">{{ $booking->author?->name ?? 'ผู้สมัคร' }}</span>
                            @php
        ...
        @empty
            <div class="text-center py-16">
                ...
                <p class="text-gray-500">{{ $filterStatus ? 'ไม่พบรายการในสถานะนี้' : 'ยังไม่มีการสมัคร' }}</p>
            </div>
        @endforelse
    </div>
