<div class="max-w-2xl mx-auto px-3 sm:px-4 py-4 md:py-6"
    ...
    <div class="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
        ...
        <h1 class="text-xl md:text-2xl font-bold text-white">✏️ แก้ไขผลงาน</h1>
    </div>

    <form wire:submit="save" class="space-y-3 md:space-y-4">
        <div class="bg-gray-900 rounded-xl md:rounded-2xl p-4 md:p-5 space-y-3 md:space-y-4">
        ...
        <div class="flex items-center justify-between pt-2">
            ...
            <div class="relative">
                <button type="button" @click="showDeleteConfirm = !showDeleteConfirm"
                        class="px-3 py-2 md:px-4 md:py-2.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl transition text-xs md:text-sm border border-red-500/30">
                    🗑️ ลบ
                </button>
                <div x-show="showDeleteConfirm" x-transition
                     class="absolute bottom-full right-0 mb-2 bg-gray-800 border border-gray-700 rounded-xl p-3 md:p-4 shadow-xl z-10 w-48 md:w-56">
                    <p class="text-white text-[10px] md:text-sm mb-2 md:mb-3">ยืนยันลบผลงานนี้?</p>
                    <div class="flex gap-2">
                        <button type="button" wire:click="deletePortfolio"
                                class="px-2.5 py-1 md:px-3 md:py-1.5 bg-red-500 hover:bg-red-400 text-white text-[10px] md:text-xs font-bold rounded-lg transition">ลบเลย</button>
                        <button type="button" @click="showDeleteConfirm = false"
                                class="px-2.5 py-1 md:px-3 md:py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-300 text-[10px] md:text-xs rounded-lg transition">ยกเลิก</button>
                    </div>
                </div>
            </div>
        </div>
