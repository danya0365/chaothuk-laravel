<div class="max-w-6xl mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-2xl font-black text-white flex items-center gap-2">🛒 การว่าจ้าง (My Hires)</h1>
        <p class="text-gray-400 mt-1">ติดตามสถานะงานที่คุณจ้างช่าง/ผู้ให้บริการ บนระบบ Chaothuk</p>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div wire:click="switchTab('pending')" class="cursor-pointer bg-gray-900 border {{ $tab === 'pending' ? 'border-orange-500 bg-orange-500/10' : 'border-gray-800 hover:border-gray-700' }} rounded-xl p-4 transition">
            <p class="text-gray-500 text-xs font-bold uppercase mb-1">รอยืนยันคิว</p>
        ...
        <div wire:click="switchTab('active')" class="cursor-pointer bg-gray-900 border {{ $tab === 'active' ? 'border-blue-500 bg-blue-500/10' : 'border-gray-800 hover:border-gray-700' }} rounded-xl p-4 transition">
            <p class="text-gray-500 text-xs font-bold uppercase mb-1">ช่างกำลังทำงาน</p>
        ...
        <div wire:click="switchTab('completed')" class="cursor-pointer bg-gray-900 border {{ $tab === 'completed' ? 'border-green-500 bg-green-500/10' : 'border-gray-800 hover:border-gray-700' }} rounded-xl p-4 transition">
            <p class="text-gray-500 text-xs font-bold uppercase mb-1">เสร็จสมบูรณ์</p>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
        @if($items->isEmpty())
            <div class="p-16 text-center">
                ...
                <h3 class="text-xl font-bold text-white mb-2">ยังไม่มีการจ้างงาน</h3>
                <p class="text-gray-500 text-sm">ค้นหาช่างและผู้เชี่ยวชาญที่คุณต้องการ แล้วกดจ้างงานได้เลย</p>
                <a href="{{ route('frontend.works') }}" class="inline-block mt-4 px-6 py-2 bg-orange-500 hover:bg-orange-400 text-white font-bold rounded-xl transition">
                    ค้นหาช่าง / บริการ 🔍
                </a>
            </div>
        @else
            <div class="divide-y divide-gray-800">
                @foreach($items as $item)
                    @if($tab === 'pending')
                        ...
                        <div class="p-5 hover:bg-gray-800/30 transition flex flex-col md:flex-row gap-4 items-start md:items-center">
                            <div class="w-12 h-12 rounded-full overflow-hidden bg-gray-800 flex-shrink-0">
                                <img src="{{ $item->work->author?->getAvatar() }}" class="w-full h-full object-cover">
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-gray-500 text-sm">ผู้ให้บริการ:</span>
                                    <span class="font-bold text-white">{{ $item->work->author?->name ?? 'User' }}</span>
                                    <span class="text-xs px-2 py-0.5 rounded-full font-bold {{ $item->booking_status === 'waiting-to-confirm' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-green-500/20 text-green-400' }}">
                                        {{ $item->booking_status === 'waiting-to-confirm' ? 'รอผู้รับงานตอบรับคิว' : 'ผู้รับงานตอบรับแล้ว (รอช่างเริ่มงาน)' }}
                                    </span>
                                </div>
                                <div class="text-sm font-medium text-white mb-1">
                                    <a href="{{ route('frontend.works.show', $item->work_id) }}" class="hover:text-orange-400 transition">{{ $item->work->title }}</a>
                                </div>
                                <div class="text-xs text-gray-500 mt-1">
                                    💰 ค่าบริการ: ฿{{ number_format($item->work->price) }} • 📅 นัดหมาย: {{ $item->booking_date ? \Carbon\Carbon::parse($item->booking_date)->format('d/m/Y') : '-' }}
                                </div>
                            </div>
                            <div class="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
                                @if($item->booking_status === 'waiting-to-confirm')
                                    <button wire:click="cancelBooking({{ $item->id }})" wire:confirm="ยกเลิกคำขอจ้างงานนี้?" class="w-full md:w-auto px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium text-sm rounded-lg transition">
                                        ยกเลิกคำขอจอง
                                    </button>
                                @endif
                            </div>
                    ...
                    @else
                        ...
                        <div class="p-5 hover:bg-gray-800/30 transition flex flex-col md:flex-row gap-4 items-start md:items-center">
                            <div class="w-12 h-12 rounded-full overflow-hidden bg-gray-800 flex-shrink-0">
                                <img src="{{ $item->worker?->getAvatar() }}" class="w-full h-full object-cover">
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-gray-500 text-sm">ผู้ให้บริการ:</span>
                                    <span class="font-bold text-white">{{ $item->worker?->name ?? 'User' }}</span>
                            ...
                            <div class="flex items-center gap-2 w-full md:w-auto mt-4 md:mt-0">
                                <a href="{{ route('frontend.sessions.show', $item->id) }}" class="w-full md:w-auto px-4 py-2 bg-gradient-to-r from-orange-600 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white font-bold text-sm rounded-lg transition text-center shadow-lg shadow-orange-500/20">
