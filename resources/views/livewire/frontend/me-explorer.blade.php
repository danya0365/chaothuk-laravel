<div class="max-w-7xl mx-auto px-4 py-10">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-white">🔮 Me Explorer</h1>
        <p class="mt-1 text-gray-400">ทดสอบ /api/me/* endpoints ด้วย Sanctum Token</p>
    </div>

    @if(!session('api_token'))
        <div class="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-8 text-center">
            <span class="text-4xl mb-4 block">🔒</span>
            <p class="text-yellow-400 font-medium mb-2">ต้องมี API Token ก่อน</p>
            <p class="text-sm text-gray-400 mb-4">ไปที่ Dashboard และ Login เพื่อรับ Sanctum Token</p>
            <a href="{{ route('frontend.dashboard') }}"
               class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-r from-orange-500 to-rose-600 text-white hover:opacity-90 transition-all">
                🏠 ไป Dashboard
            </a>
        </div>
    @else
        ...
        {{-- Tabs --}}
        <div class="flex gap-1 p-1 bg-white/5 rounded-xl border border-white/10 mb-6 overflow-x-auto">
            @foreach([
                ...
                <button wire:click="switchTab('{{ $tab['id'] }}')"
                        class="shrink-0 flex flex-col items-center px-4 py-2.5 rounded-lg text-sm transition-all
                               {{ $activeTab === $tab['id'] ? 'bg-white/10 text-white font-medium' : 'text-gray-400 hover:text-white hover:bg-white/5' }}">
                    {{ $tab['label'] }}
                    <span class="text-xs text-gray-600 font-mono mt-0.5">{{ $tab['endpoint'] }}</span>
                </button>
            @endforeach
        </div>
        ...
        @if($activeTab === 'profile' && $profile)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="rounded-2xl border border-white/10 bg-white/5 p-6">
                    ...
                    <div class="space-y-2">
                        @foreach(['location' => '📍 Location', 'mobile_phone' => '📞 Phone'] as $key => $label)
                            @if(!empty($user[$key]))
                                <div class="flex items-center gap-3 py-2 px-3 rounded-lg bg-white/5">
                                    <span class="text-xs text-gray-400">{{ $label }}</span>
                                    <span class="text-sm text-white ml-auto">{{ $user[$key] }}</span>
                                </div>
                            @endif
                        @endforeach
                    </div>
                ...
                <div class="rounded-2xl border border-white/10 bg-white/5 p-5">
                    <p class="text-xs text-gray-500 mb-2 font-mono">GET /api/me → Raw JSON</p>
                    <pre class="text-xs text-gray-300 overflow-auto max-h-72 leading-relaxed">{{ json_encode($profile, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </div>
            </div>
