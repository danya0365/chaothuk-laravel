<div class="max-w-5xl mx-auto px-4 py-6">
    ...
    <h1 class="text-2xl font-bold text-white mb-6">🏷️ หมวดหมู่</h1>

    @if($selectedCategory)
        {{-- Back + Selected Category --}}
        <div class="mb-6">
            <button wire:click="clearCategory"
                    class="flex items-center gap-2 text-gray-400 hover:text-white text-sm mb-3 transition">
        ...
        {{-- Works in Category --}}
        <div class="mb-8">
            <h3 class="font-bold text-white mb-3">📦 งาน ({{ count($works ?? []) }})</h3>
            @if(empty($works))
                <p class="text-gray-500 text-sm text-center py-6">ยังไม่มีงานในหมวดหมู่นี้</p>
            @else
        ...
        <div>
            <h3 class="font-bold text-white mb-3">👷 รับสมัคร ({{ count($recruits ?? []) }})</h3>
            @if(empty($recruits))
                <p class="text-gray-500 text-sm text-center py-6">ยังไม่มีประกาศรับสมัครในหมวดหมู่นี้</p>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
    ...
    @else
        ...
        @if(empty($categories))
            <div class="text-center py-16">
                ...
                <p class="text-gray-400 font-medium">ยังไม่มีหมวดหมู่</p>
            </div>
        @else
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                @foreach($categories as $cat)
                    <button wire:click="selectCategory({{ $cat['id'] }})"
                            class="bg-gray-900 rounded-xl p-4 text-center hover:ring-1 hover:ring-orange-500/50 transition group">
                        @if($cat['image_url'] ?? null)
                            <img src="{{ $cat['image_url'] }}"
                                 class="w-16 h-16 mx-auto rounded-lg object-cover mb-3 group-hover:scale-110 transition duration-300" alt="">
                        @else
                            <div class="w-16 h-16 mx-auto rounded-lg bg-orange-500/10 flex items-center justify-center text-3xl mb-3 group-hover:scale-110 transition duration-300">
                                🏷️
                            </div>
                        @endif
                        <p class="font-semibold text-white text-sm">{{ $cat['name'] }}</p>
                        @if($cat['description'] ?? null)
                            <p class="text-gray-500 text-xs mt-1 line-clamp-2">{{ $cat['description'] }}</p>
                        @endif
                    </button>
                @endforeach
            </div>
