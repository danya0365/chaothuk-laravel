        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-4 mx-auto sm:px-0 px-6 md:px-0 xl:px-0 lg:px-0">
                <div class="flex justify-end ">
                    <a href="{{ route('backend.issue-points.create') }}"
                            ...
                            <tbody class="bg-white dark:bg-gray-800">
                                @forelse ($issuePoints as $issuePoint)
                                    <tr>
                                            ...
                                            <dl class="divide-y divide-gray-100 dark:divide-gray-700">
                                                <div class="p-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-0">
                                                    ...
                                                    <dd
                                                        ...
                                                        @if ($issuePoint->customer)
                                                            <a
                                                                href="{{ route('backend.user-customers.show', $issuePoint->customer?->user_id) }}">
                                                                #{{ $issuePoint->customer?->user_id }}
                                                            </a>
                                                        @else
                                                            ไม่พบข้อมูลลูกค้า {{ $issuePoint->user_id }}
                                                        @endif
                                                    </dd>
                                                </div>
                                                <div class="p-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-0">
                                                    ...
                                                    <dd
                                                        class="mt-1 text-sm leading-6 text-gray-700 dark:text-gray-300 sm:col-span-2 sm:mt-0">
                                                        {{ $issuePoint->customer?->getPersonInfo()->getFullName() ?? 'ยังไม่ได้ระบุ' }}
                                                    </dd>
                                                </div>
                                                @if ($issuePoint?->cron()->isTypeWeekDayInWeek())
                                                    <div class="p-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-0">
                                                        <dt
                                                            class="text-sm font-medium leading-6 text-gray-900 dark:text-gray-100">
                                                            ทำซ้ำ</dt>
                                                ...
                                                @endif
                                                @if ($issuePoint?->cron()->isTypeDateInMonth())
                                                    <div class="p-2 sm:grid sm:grid-cols-3 sm:gap-2 sm:px-0">
                                                        <dt
                                                            class="text-sm font-medium leading-6 text-gray-900 dark:text-gray-100">
                                                            ทำซ้ำ</dt>
                                                        <dd
                                                            class="mt-1 text-sm leading-6 text-gray-700 dark:text-gray-300 sm:col-span-2 sm:mt-0">
                                                            ทุกวันที่ {{ $issuePoint?->cron()->getRepeatValue() }}
                                                            ของเดือน
                                                        </dd>
                                                    </div>
                                                @endif
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 dark:border-gray-600">
                                            <form action="{{ route('backend.issue-points.destroy', $issuePoint->id) }}"
                                                method="POST">

                                                <a class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-200"
                                                    href="{{ route('backend.issue-points.show', $issuePoint->id) }}">
                                                ...
                                                <a class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-200"
                                                    href="{{ route('backend.issue-points.status-form', $issuePoint->id) }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 inline-block"
                                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
