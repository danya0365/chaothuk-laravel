...
class MockSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Make sure locally-generated mock images are in public storage
        // (generated by `php artisan mock-images:generate` via local ComfyUI)
        ...
        // Assign local mock images (ComfyUI-generated pool) by work type
        \App\Services\MockImageService::assignToModels($works, 'work');
        ...
        // Assign local mock images by work type
        \App\Services\MockImageService::assignToModels($recruits, 'recruit');
        ...
        foreach ($bannerNames as $i => $name) {
            $bannerSlug = 'banner-' . ($i + 1);
            Banner::create([
                'name'         => $name,
                'type'         => BannerType::EXTERNAL_URL->value,
                'image_url'    => \App\Services\MockImageService::publicRel('banner', $bannerSlug),
        ...
        foreach ($workerIds as $wIdx => $wId) {
            ...
            for ($p = 0; $p < $numPortfolios; $p++) {
                $slugA = $portfolioSlugs[$pCounter % count($portfolioSlugs)];
                $slugB = $portfolioSlugs[($pCounter + 1) % count($portfolioSlugs)];
                $pCounter++;
                Portfolio::create([
                    'user_id'      => $wId,
                    ...
                    'images'       => [
                        \App\Services\MockImageService::publicRel('portfolio', $slugA),
                        \App\Services\MockImageService::publicRel('portfolio', $slugB),
                    ],
